    ...
    for path in glob.iglob(target):
        ...
        for object in objects:
            ...
            for key in range(len(offsetMap)):
                ...
                if transformed_area / float(org_area) < thresh:
                    continue
        ...
        for f, x in xmlMap.items():
            xml_list.append(f)
            x.write(os.path.join(output_dir, f), encoding='utf-8')
        ...
        for window_size, bb_list in wsMap.items():
            ...
            for i in range(num_vert):
                for j in range(num_horizon):
                    ...
                    cv2.putText(sample_img, str(i) + '-' + str(j), (int((width_end - width_start)/3) + width_start, int((height_end - height_start)/2) + height_start), cv2.FONT_HERSHEY_PLAIN, 1, square_block_color, square_block_width, cv2.LINE_AA)
                    ...
                    if newfile + '.xml' in xml_list:
                        ...
                        cv2.imwrite(os.path.join(output_dir, newfile + '.' + args.image_format), clopped)

            for bb in bb_list:
                cv2.rectangle(sample_img, (bb[0], bb[1]), (bb[2], bb[3]), bndbox_line_color, bndbox_line_width)

            if args.create_sample:
                cv2.imwrite(os.path.join(output_dir, 'sample-' + basefilename + '-' + 'W' + str(window_size) + '.jpg'), sample_img)
